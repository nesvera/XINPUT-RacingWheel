...
int16_t xViewValue = 0;
int16_t yViewValue = 0;
...
void declareButtonPins(){
	digitalArray[0].port  = NULL;			digitalArray[0].pin  = NULL;					digitalArray[0].state  = 0;		// A ( pin is analog for the handbreak )
...
void declareAnalogPins(){
	analogArray[0].port = GPIOA;		analogArray[0].pin = GPIO_PIN_4;		analogArray[0].state = 0;			// A ( fake analog - handbreak  )
	analogArray[1].port = GPIOA;		analogArray[1].pin = GPIO_PIN_0;		analogArray[1].state = 0;			// LEFT TRIGGER
	analogArray[2].port = GPIOA;		analogArray[2].pin = GPIO_PIN_1;		analogArray[2].state = 0;			// RIGHT TRIGGER
	analogArray[3].port = NULL;			analogArray[3].pin = NULL;					analogArray[3].state = 0;			// X LEFT STICK		( ENCODER: INTERRUPT PIN and DIGITAL )
	analogArray[4].port = NULL;			analogArray[4].pin = NULL;					analogArray[4].state = 0;			// Y LEFT STICK		( NOT USED )
	analogArray[5].port = GPIOA;		analogArray[5].pin = GPIO_PIN_2;		analogArray[5].state = 0;			// X RIGHT STICK
	analogArray[6].port = GPIOA;		analogArray[6].pin = GPIO_PIN_3;		analogArray[6].state = 0;			// Y RIGHT STICK
}
...
void declareEncoderPins(){
	encoderPins[0].port = GPIOB;		encoderPins[0].pin = GPIO_PIN_8;		encoderPins[0].state = 0;			// Encoder input A - interrupt pin
	encoderPins[1].port = GPIOB;		encoderPins[1].pin = GPIO_PIN_7;		encoderPins[1].state = 0;			// Encoder input B - normal input
}
...
void readButtons(){
	...
	uint16_t auxHandbrakeValue = handbrakeValue;						// get value from handbreak adc
	
	// calculate handbrake
	// handbrake has X degrees of travel
	if( auxHandbrakeValue < MID_HANDBRAKE_POS ){													// handbrake dont work until half travel
		buttonArray[0] = 0;
		
	}else if( auxHandbrakeValue > MAX_HANDBRAKE_POS ){										// It stays active when reach the limit travel 
		buttonArray[0] = 1;
		
	}else{																																// between half and end, the handbrake works randomly
		uint16_t maxRand = (MAX_HANDBRAKE_POS - auxHandbrakeValue)/5;
		uint16_t randValue = rand() % maxRand;
...
void updateTriggers(){
	...
	if( adcValueReady == 1 ){
		throttle 	= (uint8_t)map( rightTriggerValue, 0, MAX_THROTTLE, 0, UINT8_MAX );
		brake 		= (uint8_t)map( leftTriggerValue, 0, MAX_BRAKE, 0, UINT8_MAX );
			
		xViewValue = (int16_t)map( xRightStickValue, 0, 4040, INT16_MIN, INT16_MAX );
		yViewValue = (int16_t)map( yRightStickValue, 0, 4040, INT16_MIN, INT16_MAX );
		
		adcValueReady = 0;
	}
		
	XINPUT_triggerUpdate(brake, throttle);
}
...
void updateSticks(){
	...
	if( wheelEncoderValue > (STEERING_DEGREES/2 - 1) ){
		value = INT16_MAX;
	}else if ( wheelEncoderValue < (-STEERING_DEGREES/2 + 1) ){
		value = INT16_MIN;
	}else{
		value = (int16_t)map(wheelEncoderValue, -STEERING_DEGREES/2, STEERING_DEGREES/2, INT16_MIN, INT16_MAX );
	}
		
	XINPUT_stickUpdate(STICK_LEFT, value, 0);
	XINPUT_stickUpdate(STICK_RIGHT, xViewValue, yViewValue );
